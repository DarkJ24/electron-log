...
var findLogPathWithDate = require('./find-log-path-with-date');
...
transport.date         = getDateFormatted(new Date());
transport.useDate      = false;
...
function transport(msg) {
  ...
  var needLogRotation = (transport.maxSize > 0 &&
    ...
    (transport.useDate && transport.date !== getDateFormatted(new Date()));

  if (needLogRotation) {
    if ((transport.useDate && transport.date !== getDateFormatted(new Date()))){
      archiveLogDate(transport.stream);
    } else {
...
function initSteamConfig() {
  if (transport.useDate) {
    transport.file = findLogPathWithDate(transport.appName,getDateFormatted(new Date()));
    transport.date = getDateFormatted(new Date());
  } else {
...
function archiveLogDate(stream) {
  if (stream.end) {
    stream.end();
  }
  transport.file = findLogPathWithDate(transport.appName, getDateFormatted(new Date()));
