...
function transport(msg) {
  ...
  var needLogRotation = (transport.maxSize > 0 &&
    ...
    (transport.useDate && transport.file !== getDateFormatted(new Date()));

  if (needLogRotation) {
    if ((transport.useDate && transport.file !== getDateFormatted(new Date()))){
...
function initSteamConfig() {
  if (transport.useDate) {
    transport.file = findLogPath(transport.appName+'/'+getDateFormatted(new Date()));
  } else {
